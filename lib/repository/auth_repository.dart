...
class AuthRepository {
  Future loginUser(String _email, String _password, String device) async {
    String baseUrl = "http://127.0.0.1/api/auth/login";
  ...
  Future userLogout(String token) async {
    String baseUrl = "http://127.0.0.1/api/auth/logout";
  ...
  Future getData(String token) async {
    String baseUrl = "http://127.0.0.1/api/auth/me";
  ...
  Future hasToken() async {
    ...
    final String? token = local.getString("token-sanctum") ?? null;
    if (token != null) {
      return token;
    }
    return null;
